...
public class NextSmallerElement {
    ...
    /**
     * Finds the nearest smaller element on the right side for every element
     * in the given array.
     *
     ...
     * because they can never become a next smaller element for the
     * current element or any element positioned further to the left.
     *
     ...
     * @return Array containing the next smaller element for each position.
     */
    static int[] nextSmaller(int[] arr) {
        ...
        for (int i = arr.length - 1; i >= 0; i--) {
            ...
            // Remove elements that cannot act as a next smaller element.
            while (!stack.isEmpty() && arr[stack.peek()] >= arr[i]) {
                stack.pop();
            }

            // Stack top represents the nearest smaller element on the right.
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "Next Smaller Series: "
                        + Arrays.toString(nextSmaller(arr)));
    }
}
